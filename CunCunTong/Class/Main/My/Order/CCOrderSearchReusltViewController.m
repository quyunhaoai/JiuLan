...
@interface CCOrderSearchReusltViewController ()<SegmentTapViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation CCOrderSearchReusltViewController
...
- (void)setupUI {
    ...
    self.searBarView.searchTextField.placeholder = self.search;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == model.goods_order_set.count+1) {
        ...
        if ( model.status == 0) {
            ...
            cell.line1View.hidden = NO;
            cell.sureBtn.hidden = NO;
        }else if(model.status == 2 || model.status == 5){
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CCMyOrderModel *model = [CCMyOrderModel modelWithJSON:self.dataSoureArray[indexPath.section]];
    if (indexPath.row == model.goods_order_set.count+1) {
        if ( model.status == 0 || model.status == 3 || model.status == 4 ||model.status == 2 || model.status == 5) {
            return 120;
        }
        return 60;
    }
...
- (void)commentBtnClick:(UIButton *)button {
    if ([button.titleLabel.text isEqualToString:@"立即支付"]) {
        ...
        NSString *path = [NSString stringWithFormat:@"/app0/cancleorder/%ld/",model.ccid];
        [[STHttpResquest sharedManager] requestWithPUTMethod:PUT
                                                    WithPath:path
                                                  WithParams:params
                                            WithSuccessBlock:^(NSDictionary * _Nonnull dic) {
        ...
        NSString *path =[NSString stringWithFormat:@"/app0/myorder/%ld/",model.ccid];
