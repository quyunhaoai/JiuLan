...
- (NSString *)subTitleForIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        switch (indexPath.row)
        {
            ...
            case 3: subTitle = [NSString stringWithFormat:@"%.2f(元)", self.path.tolls]; break;
            ...
            case 6: subTitle = [NSString stringWithFormat:@"%ld", (long)self.path.restriction]; break;
...
- (void)viewDidLoad
{
    ...
    _baseInfoTitle = @[@"起点和终点的距离", @"预计耗时", @"导航策略", @"费用", @"收费路段长度", @"交通信号灯数量", @"限行信息"];
