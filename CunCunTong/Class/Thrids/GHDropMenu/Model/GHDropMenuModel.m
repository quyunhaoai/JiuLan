...
- (NSArray *)creaFilterDropMenuData {
    ...
    /** 构造右侧弹出筛选菜单第一行数据 */
    NSMutableArray *row1 = [NSMutableArray arrayWithArray:self.brand_set];
    NSMutableArray *dataArray4 = [NSMutableArray array];
    for (NSInteger index = 0 ; index < row1.count; index++) {
        GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
        NSDictionary *dict = [row1 by_ObjectAtIndex:index];
        dropMenuModel.tagName = dict[@"brand_name"];
        dropMenuModel.tagIdentifier = [dict[@"brand_id"] integerValue];
        if (index == 0) {
    ...
    /** 构造右侧弹出筛选菜单第二行数据 */
    NSMutableArray *row2 = [NSMutableArray arrayWithArray:self.package_set];
    NSMutableArray *dataArray5 = [NSMutableArray array];
    for (NSInteger index = 0 ; index < row2.count; index++) {
        GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
        NSDictionary *dic = [row2 by_ObjectAtIndex:index];
        dropMenuModel.tagName = dic[@"package"];
        if (index == 0) {
            dropMenuModel.tagSeleted = YES;
        } else {
            dropMenuModel.tagSeleted = NO;
    ...
    for (NSDictionary *dic in self.spec_set) {
        GuGeModelName *model = [GuGeModelName modelWithJSON:dic];
        NSMutableArray *dataArray = [NSMutableArray array];
        for (NSInteger index = 0 ; index < model.children.count; index++) {
            GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
            ChildrenItem *mmm =[ChildrenItem modelWithJSON:[model.children by_ObjectAtIndex:index]];
            dropMenuModel.tagName = mmm.specoption_name;
            dropMenuModel.tagIdentifier = mmm.specoption_id;
            if (index == 0) {
                dropMenuModel.tagSeleted = YES;
            } else {
                dropMenuModel.tagSeleted = NO;
        ...
        [titleArr addObject:model.spec_name];
        [modelArr addObject:dataArray];
    }
    /** 设置构造右侧弹出筛选菜单每行的标题 */
    NSMutableArray *sectionHeaderTitles =[NSMutableArray arrayWithArray:@[@"品牌",@"包装种类",@"价格"]];
    [sectionHeaderTitles addObjectsFromArray:titleArr];
    NSMutableArray *sections = [NSMutableArray array];
    ...
    for (NSInteger index = 0; index < sectionHeaderTitles.count; index++) {
        ...
        if (index == 1) {
            ...
            dropMenuModel.dataArray = modelArr[index-3];
            /** 单选 */
            dropMenuModel.isMultiple = NO;
            dropMenuModel.filterCellType = GHDropMenuFilterCellTypeTag;
        }
